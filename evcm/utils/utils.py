...
def mat2file(fname,Au,Al,S,Gu,Gl,beta,Sigmau,Sigmal,uimmutable,limmutable):
    np.savez(fname,Au=Au,Al=Al,S=S,Gu=Gu,Gl=Gl,beta=beta,Sigmau=Sigmau,Sigmal=Sigmal,uimmutable=uimmutable,limmutable=limmutable)
...
def random_network_biomass_beta(m,r,n_import=1,n_export=1,z_max=4,nu_max=2,biomass_rxt=None):
    ...
    while True:
        ...
        for (nmr,i) in zip(num_metabolites_reactions,range(n_exchange,n_exchange+len(num_metabolites_reactions))):
            ...
            #We have enough unused metabolites to fill the current reaction
                ...
                ri = metabolites_not_used[ri_idx] #Metabolites involved in this reaction
            ...
            #pick indices, we just know one is consumed and one is produced
                ...
                ri = np.random.choice(m,1) #Pick another random metabolite to be in the reaction
                ...
                ri_idx = np.random.choice(len(metabolites_not_used),len(metabolites_not_used),replace=False)
                # print(ri_idx)
        ...
        if biomass_rxt is None:
            ...
            biomass_rxt = int(m*0.1) #Won't use every reactant in the biomass reaction 
            biomass_idx = np.random.choice(m,biomass_rxt,replace=False)
            S[biomass_idx,-1] = np.random.randn(biomass_rxt)
            S[biomass_idx,-1] = (S[biomass_idx,-1] - np.mean(S[biomass_idx,-1])).round(3)
...
def random_start(Au,Al,S,Gu,Gl,beta,uimmutable=[],limmutable=[],scale=1,irreversible=False):
    while True:
        ...
        flux = nearest_feasible_gene(ubounds,lbounds,flux,Au,Al,S,Gu,Gl,irreversible=irreversible)
...
def fixed_start(Au,Al,S,Gu,Gl,beta,uimmutable=[],limmutable=[],scale=1,irreversible=False):
    ubounds = np.ones(Gu.shape[1])*scale
    ubounds[[i[0] for i in uimmutable]] = [i[1] for i in uimmutable]
    lbounds = -np.ones(Gl.shape[1])*scale
    lbounds[[i[0] for i in limmutable]] = [-i[1] for i in limmutable]
    biomass,flux = FBA_gene(ubounds,lbounds,Au,Al,S,Gu,Gl,beta,irreversible=irreversible)

    return ubounds, lbounds, flux, biomass

def fixednoisy_start(Au,Al,S,Gu,Gl,beta,epi=0.005,uimmutable=[],limmutable=[],scale=1,irreversible=False):
...
def biological_start(Au,Al,S,Gu,Gl,beta,epi=0.005,uimmutable=[],limmutable=[],scale=1,irreversible=False):
    ...
    biomass0,flux0 = FBA_gene(ubounds,lbounds,Au,Al,S,Gu,Gl,beta,irreversible=irreversible)
    ...
    while True: #Add some noise to the bounds, but check to make sure nothing is infeasible
        ...
        biomass_bio,flux_bio = FBA_gene(ubounds_bio,lbounds_bio,Au,Al,S,Gu,Gl,beta,irreversible=irreversible)
...
def mutate_bounds(x,p,scale=0.1,immutable=[],Sigma=None):
    r = len(x)
    if Sigma is None:
        Sigma = np.eye(r)

    effects = np.random.multivariate_normal(mean=np.zeros(r),cov=Sigma)*scale
...
def selective_pressure_flux(Au,Al,S,Gu,Gl,beta,v0,u_g,l_g,u_comp,l_comp,delta=1e-6,push=1):
...
def selective_pressure_gene(Au,Al,S,Gu,Gl,beta,v0,u_g,l_g,u_comp,l_comp,delta=1e-6,push=1):
    ...
    for i_u in range(len(u_g)):
        ...
        u_g[i_u] += delta
        new_flux = nearest_feasible_gene(u_g,l_g,v0+push*delta*b,Au,Al,S,Gu,Gl)
        SP_u[i_u] = (beta.T @ new_flux - beta.T @ v0)/delta
        u_g[i_u] -= delta
...
def selective_pressure_FBA_gene(Au,Al,S,Gu,Gl,beta,v0,u_g,l_g,u_comp,l_comp):
    ...
    _,_,_,SP_u,SP_l = FBA_gene(u_g,l_g,Au,Al,S,Gu,Gl,beta,return_lagrange=True)
...
def integrateSP(SP,mask=None):
    ...
    SP_pivot = pd.pivot(data=SP,columns='Vector',index='Time',values='Selective Pressure')
    
    if mask is not None:
        
        SP_int = cumulative_trapezoid(SP_pivot[mask].to_numpy(),x=SP_pivot.index[mask],axis=0,initial=0)
        return pd.DataFrame(data=SP_int,index=SP_pivot.index[mask],columns=SP_pivot.columns)
        
    else:
        
        SP_int = cumulative_trapezoid(SP_pivot.to_numpy(),x=SP_pivot.index,axis=0,initial=0)
...
def optimal_fitness(df_flux,df_ubounds,df_lbounds,Au,Al,S,Gu,Gl,beta):
    ...
    np_ubounds = pd.pivot(data=df_ubounds,columns='Gene',index='Time',values='Bound').to_numpy()
    np_lbounds = pd.pivot(data=df_lbounds,columns='Gene',index='Time',values='Bound').to_numpy()
    np_flux = pd.pivot(data=df_flux,columns='Reaction',index='Time',values='Relative Flux').to_numpy()

    optimal_flux = np.zeros(np_flux.shape)
    
    for i in range(np_ubounds.shape[0]):
        
        _,optimal_flux[i,:] = FBA_gene(np_ubounds[i,:],np_lbounds[i,:],Au,Al,S,Gu,Gl,beta)
...
def FBAlamvsest(Au,Al,S,Gu,Gl,beta,u_g,l_g,delta=0.001):
    ...
    f0,_,_,lam_u_FBA,lam_l_FBA = FBA_gene(u_g,l_g,Au,Al,S,Gu,Gl,beta,return_lagrange=True)
