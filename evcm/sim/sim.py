import numpy as np
import pandas as pd
from evcm.utils import (
    fixed_start,
    fixednoisy_start,
    random_start,
    sample_flux_p,
    random_fixation,
    nearest_feasible_gene,
    mutate_bounds,
    selective_pressure_flux,
    selective_pressure_gene,
    FBA_gene_container,
...
def run_sim(
    ...
    uGene_labels=None,
    lGene_labels=None,
    ucComparison_labels=None,
    lcComparison_labels=None,
    ugComparison_labels=None,
    lgComparison_labels=None,
    u_g0=None,
    l_g0=None,
    v0=None,
    mutation_data="c",
    ...
    if penalty == __G__:
        fitness = (
            lambda v, gu, gl: beta.dot(v)
            - alpha * (gu**pen_power).sum()
            - alpha * ((-gl) ** pen_power).sum()
        )
    elif penalty == __V__:
    ...
    if fix_start == __fix__:
        u_g, l_g, flux, biomass = fixed_start(
            Au,
            Al,
            S,
            Gu,
        ...
        u_g, l_g, flux, biomass = random_start(
            Au,
            Al,
            S,
            Gu,
        ...
        u_g, l_g, flux, biomass = fixednoisy_start(
            Au,
            Al,
            S,
            Gu,
        ...
        u_g, l_g, flux, biomass = biological_start(
            Au,
            Al,
            S,
            Gu,
    ...
    for t in range(T):
        ...
        new_u_g = mutate_bounds(
            u_g,
            mutate_prob,
            scale=mutate_scale * simulation_scale,
            immutable=uimmutable,
            Sigma_cholesky=Sigmau_cholesky,
        )
        new_l_g = -mutate_bounds(
            -l_g,
            mutate_prob,
            scale=mutate_scale * simulation_scale,
            immutable=limmutable,
            Sigma_cholesky=Sigmal_cholesky,
        )

        if fba_move:
            _, new_flux = FBA_container.optimize(new_u_g, new_l_g)
        else:
            new_flux = sample_flux_p(
                flux, heritability_std * simulation_scale, wiggle_prob
            )
            new_flux = nearest_feasible_gene(
                new_u_g, new_l_g, new_flux, Au, Al, S, Gu, Gl, irreversible=irreversible
            )
        ...
        if (new_biomass >= 0) and (np.random.random() < prob_fix):
            ...
            if mutation_data == __full__ or (
                mutation_data == __neutral__ and selective_coeff.round(8) == 0
            ):
        ...
        if t % SP_rate == 0 and do_SP_flux:
            ...
            # Use FBA
            SP_uc_fba, SP_lc_fba = selective_pressure_FBA_flux(
                Au, Al, S, Gu, Gl, beta, flux, u_g, l_g, u_ccomp, l_ccomp
            )
            ...
            # Do it "empirically"
            SP_uc_emp, SP_lc_emp = selective_pressure_flux(
                Au, Al, S, Gu, Gl, beta, flux, u_g, l_g, u_ccomp, l_ccomp
            )
        ...
        if t % SP_rate == 0 and do_SP_gene:
            ...
            # Use FBA
            SP_ug_fba, SP_lg_fba = selective_pressure_FBA_gene(
                Au, Al, S, Gu, Gl, beta, flux, u_g, l_g, u_gcomp, l_gcomp
            )
            ...
            # Do it "empirically"
            SP_ug_emp, SP_lg_emp = selective_pressure_gene(
                Au, Al, S, Gu, Gl, beta, flux, u_g, l_g, u_gcomp, l_gcomp
            )
        ...
        if mutation_data == __count__:
            ...
            df_neutral = {
                "Gene": uGene_labels + lGene_labels,
                "UpperLower": ["Upper"] * len(uGene_labels)
                + ["Lower"] * len(lGene_labels),
                "Neutral Count": list(neutral_umutation_count)
                + list(neutral_lmutation_count),
            }

    return (
        pd.DataFrame(df_flux),
        pd.DataFrame(df_ubounds),
        pd.DataFrame(df_lbounds),
        pd.DataFrame(df_biomass),
        ...
        pd.DataFrame(df_lgSP_fba),
        pd.DataFrame(df_ucSP_emp),
        pd.DataFrame(df_lcSP_emp),
        pd.DataFrame(df_ugSP_emp),
        pd.DataFrame(df_lgSP_emp),
